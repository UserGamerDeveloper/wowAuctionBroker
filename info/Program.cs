...
namespace info
{
    public class Program
    {
        ...
        //    new Server((int)ServerId.gorduni, gorduni,new List<int>{ (int)ItemId.LEATHER, (int)ItemId.SCALE,(int)ItemId.SILK,(int)ItemId.Tidespray_Linen},"гордунни",18,0)
        //};
        static XmlSerializer serverXmlSerializer;
        static Dictionary<int, Item> itemsDictionary;
        ...
        {
            serverXmlSerializer = new XmlSerializer(typeof(Server[]));
            ...
            //{
            //    new Item(124437, "ткань", COST_Shaldorei_Silk, SILK, TIME_NEED),
            //    new Item(124113, "кожа", COST_LEATHER, LEATHER, TIME_NEED),
            //    new Item(124115, "чещуя", COST_SCALE, SCALE, TIME_NEED),
            //    new Item(152576, "лен", COST_Tidespray_Linen, Tidespray_Linen, TIME_NEED),
            //    new Item(ID_Shimmerscale, "Shimmerscale", 0, Shimmerscale, 0),
            //    new Item(ID_BloodStainedBone, "BloodStainedBone", 0, BloodStainedBone, TIME_NEED_BloodStainedBone),
            //    new Item(ID_CoarseLeather, "CoarseLeather", 0, CoarseLeather, 0)
            //};
            //using (FileStream fs = new FileStream("items.xml", FileMode.Create))
            //{
            //    XmlSerializer xmlSerializer = new XmlSerializer(typeof(Item[]));
            //    xmlSerializer.Serialize(fs, itemss);
            //}
            itemsDictionary = new Dictionary<int, Item>();
            using (FileStream fs = new FileStream("items.xml", FileMode.Open))
            {
                XmlSerializer xmlSerializer = new XmlSerializer(typeof(Item[]));
                foreach (Item item in (Item[])xmlSerializer.Deserialize(fs))
                {
        ...
        private static void parseServer()
        {
            for (int idServer = servers.Length - 1; idServer >= 0; idServer--)
            {
                ...
                if (server.isUpdate())
                {
                    ...
                    Dictionary<int, Item> items = cloneDictionary(itemsDictionary);
                    ...
                    AuctionPageHTMLParser parserBloodStainedBone = new AuctionPageHTMLParser(items[(int)ItemId.BloodStainedBone].getUri(), server.cookie);
                    AuctionPageHTMLParser parserCoarseLeather = new AuctionPageHTMLParser(items[(int)ItemId.CoarseLeather].getUri(), server.cookie);
                    AuctionPageHTMLParser parserShimmerscale = new AuctionPageHTMLParser(items[(int)ItemId.Shimmerscale].getUri(), server.cookie);
                            ...
                            for (; indexBloodStainedBone < bidCount; indexBloodStainedBone++)
                            {
                                ...
                                if (costPerItem > 0)
                                {
                                    ...
                                    bidsBloodStainedBone.Add(
                                        new Bid(
                                            (int)ItemId.BloodStainedBone,
                                            countInBid,
                                            costPerItem,
                                            parserBloodStainedBone.getAutor(indexBloodStainedBone),
                                            0,
                                            items[(int)ItemId.BloodStainedBone].TIME_NEED
                                        )
                                    );
                                    ...
                                    for (; indexCoarseLeather < bidCount; indexCoarseLeather++)
                                    {
                                        ...
                                        if (costPerItem > 0)
                                        {
                                            ...
                                            bidsCoarseLeather.Add(
                                                new Bid(
                                                    (int)ItemId.CoarseLeather,
                                                    countInBid,
                                                    costPerItem,
                                                    parserCoarseLeather.getAutor(indexCoarseLeather),
                                                    0,
                                                    items[(int)ItemId.CoarseLeather].TIME_NEED
                                                )
                                            );
                                    ...
                                    for (; indexShimmerscale < bidCount; indexShimmerscale++)
                                    {
                                        ...
                                        if (costPerItem > 0)
                                        {
                                            ...
                                            bidsShimmerscale.Add(
                                                new Bid(
                                                    (int)ItemId.Shimmerscale,
                                                    countInBid,
                                                    costPerItem,
                                                    parserShimmerscale.getAutor(indexShimmerscale),
                                                    0,
                                                    items[(int)ItemId.Shimmerscale].TIME_NEED
                                                )
                                            );
                                    ...
                                    if ((countShimmerscale / 10f == countRecipe) && (countShimmerscale > 0))
                                    {
                                        ...
                                        if ((profitShimmerscaleStriker > 0) || (profitCoarseLeatherCestus > 0))
                                        {
                                            if (profitShimmerscaleStriker > profitCoarseLeatherCestus)
                                            {
                                                ...
                                                items[(int)ItemId.Shimmerscale].bids.AddRange(bidsShimmerscale);
                                            }
                                            else
                                            {
                                                setProfitInBids(bidsBloodStainedBone, profitCoarseLeatherCestus);
                                                items[(int)ItemId.CoarseLeather].bids.AddRange(bidsCoarseLeather);
                                            }
                    ...
                    foreach (int idItem in server.activeItems)
                    {
                        Item itemData = items[idItem];
        ...
        private static Dictionary<int, Item> cloneDictionary(Dictionary<int, Item> itemsDictionary)
        {
